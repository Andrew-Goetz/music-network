...
template <typename T>
std::vector<T> BST_ref<T>::getArray()
{
...
//Complexity: O(logn)
template <typename T>
//typename BST_ref<T>::Node* BST_ref<T>::find_Succsor(Node* head) {
auto BST_ref<T>::find_Succsor(Node* head) -> Node* 
{
...
T& BST_ref<T>::find(T& target) {
    ...
    if(temp == nullptr) throw outofBoundsException();
	return temp->getData();
}

//Complexity: O(logn)
template <typename T>
void BST_ref<T>::insert(T& element)
{
	//if root is nullptr
	if(root == nullptr)
	{
...
template <typename T>
//typename BST_ref<T>::Node* BST_ref<T>::delete_recursive(Node* head, T& target)
auto BST_ref<T>::delete_recursive(Node* head, T& target) -> Node*
{
	//base case
...
//Complexity: O(logn)
template <typename T>
bool BST_ref<T>::remove(T& target)
{
