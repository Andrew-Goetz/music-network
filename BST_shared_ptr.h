#ifndef BST_SHARED_PTR_H
#define BST_SHARED_PTR_H
...
template <typename T>
class BST_shared_ptr {
	public:
		class Node {
			public:
				friend class BST_shared_ptr;
				Node(std::shared_ptr<T> curData, Node* curleft, Node* curright) : 
				data{curData},
				left{curleft},
				right{curright}{};
				...
				void setData(std::shared_ptr<T> data) {this->data = data;}
		...
		BST_shared_ptr() : root{nullptr}, size{0}{};
		~BST_shared_ptr(){delete root;}
		//idk this might be useful
		std::shared_ptr<T> find(T);
		void insert(T);
		void insert(std::shared_ptr<T>);
		bool remove(T);
...
//complexity: O(n)
template <typename T>
std::vector<std::shared_ptr<T>> BST_shared_ptr<T>::getArray()
{
	std::vector<Node*> node = std::vector<Node*>();
	std::vector<std::shared_ptr<T>> ret = std::vector<std::shared_ptr<T>>();
	node.push_back(root);
	while(node.size() > 0)
	{
...
//Complexity: O(logn)
template <typename T>
//typename BST_shared_ptr<T>::Node* BST_shared_ptr<T>::find_Succsor(Node* head) {
auto BST_shared_ptr<T>::find_Succsor(Node* head) -> Node* 
{
...
template <typename T>
std::shared_ptr<T> BST_shared_ptr<T>::pop()
{
	if(root == nullptr) return std::shared_ptr<T>(nullptr);
	std::shared_ptr<T> ret = root->getData();
	remove(*ret);
	return ret;
}
...
//Complexity: O(logn)
template <typename T>
std::shared_ptr<T> BST_shared_ptr<T>::find(T target) {
	//if root is nullptr
	if (root == nullptr) {
		auto ret = std::shared_ptr<T>(nullptr);
		return ret;
	}
	...
	return temp == nullptr ? std::shared_ptr<T>() : temp->getData();
}

//Complexity: O(logn)
template <typename T>
void BST_shared_ptr<T>::insert(T element)
{
	//if root is nullptr
	if(root == nullptr)
	{
		Node *newNode = new Node(std::shared_ptr<T>(new T(element)), nullptr, nullptr);
		root = newNode;
		size++;
		return;
	}
	...
	if(storeLeft) 
	{
		temp->setLeft(new Node(std::shared_ptr<T>(new T(element)), nullptr, nullptr));
		size++;
	}
	else 
	{
		temp->setRight(new Node(std::shared_ptr<T>(new T(element)), nullptr, nullptr));
		size++;
	}
}

template <typename T>
void BST_shared_ptr<T>::insert(std::shared_ptr<T> element)
{
	//if root is nullptr
	if(root == nullptr)
	{
...
template <typename T>
//typename BST_shared_ptr<T>::Node* BST_shared_ptr<T>::delete_recursive(Node* head, T target)
auto BST_shared_ptr<T>::delete_recursive(Node* head, T target) -> Node*
{
	//base case
...
//Complexity: O(logn)
template <typename T>
bool BST_shared_ptr<T>::remove(T target)
{
